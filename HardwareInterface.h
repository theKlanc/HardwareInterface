...
namespace HI2 {
	  ...
	  private:
		...
		class Font {
		  ...
		  private:
			...
			friend void drawText(Font &font, std::string text, point2D pos, int size, Color color);
		};
		...
		class Texture {
		  ...
		  private:
			...
			friend void drawTexturePart(Texture &texture, point2D texturePartStart, int sizeX, int sizeY, point2D displayPos);
			friend Texture mergeTextures(Texture &originTexture, Texture &destinationTexture, point2D position);
		};
		...
		enum BUTTON {
			...
			KEY_UP = KEY_DUP | KEY_LSTICK_UP |
					 KEY_RSTICK_UP, ///< D-Pad Up or Sticks Up
			...
			KEY_SL =
				KEY_SL_LEFT | KEY_SL_RIGHT, ///< SL on Left or Right Joy-Con
			KEY_SR =
				KEY_SR_LEFT | KEY_SR_RIGHT, ///< SR on Left or Right Joy-Con
		};
		enum JOYSTICK {
			JOY_LEFT,
			JOY_RIGHT,
		...
		void drawTexturePart(Texture &texture, point2D texturePartStart,
							 int sizeX, int sizeY, point2D displayPos);
		Texture mergeTextures(Texture &originTexture,
							  Texture &destinationTexture, point2D position);
	...
	namespace HardwareInterface {
		...
		enum HI_KEYS {
			...
			HI_KEY_UP =
				HI_KEY_DUP | HI_KEY_CPAD_UP, ///< D-Pad Up or Circle Pad Up
			HI_KEY_DOWN = HI_KEY_DDOWN |
						  HI_KEY_CPAD_DOWN, ///< D-Pad Down or Circle Pad Down
			HI_KEY_LEFT = HI_KEY_DLEFT |
						  HI_KEY_CPAD_LEFT, ///< D-Pad Left or Circle Pad Left
			HI_KEY_RIGHT =
				HI_KEY_DRIGHT |
		...
		void drawText(HIFont font, std::string text, int posX, int posY,
		...
		void drawImage(HIImage image, int sizeX, int sizeY, int posX,
					   int posY); // HIImage image should be a pointer to an
								  // array of RGBA8 pixels
		void drawTexturePart(HITexture texture, int startX, int startY,
							 int posX, int posY, int sizeX, int sizeY);
		void drawTextureRotate(HITexture texture, int posX, int posY,
							   float angle);
		void mergeTextures(HITexture originTexture,
						   HITexture destinationTexture, short posX,
						   short posY);
