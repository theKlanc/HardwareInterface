#ifdef _VITA
#error lmao locu que _VITA esta definit buatdefac
...
void HI::setBackgroundColor(HIColor color) { vita2d_set_clear_color(color); }
...
void HardwareInterface::drawText(HIFont font, string text, int posX, int posY,
								 int size, HIColor color) {
	vita2d_font_draw_text((vita2d_font *)font, posX * SCALE, posY * SCALE,
						  color, size * SCALE, text.c_str());
}
...
void HI::drawTexture(HI::HITexture texture, int posX, int posY) {
	vita2d_draw_texture_scale((vita2d_texture *)texture, posX * SCALE,
							  posY * SCALE, SCALE, SCALE);
}

void HI::drawTextureRotate(HI::HITexture texture, int posX, int posY,
						   float angle) {
	vita2d_draw_texture_scale_rotate((vita2d_texture *)texture, posX * SCALE,
									 posY * SCALE, SCALE, SCALE, angle);
}

void HI::drawTexturePart(HI::HITexture texture, int startX, int startY,
						 int posX, int posY, int sizeX, int sizeY) {
	vita2d_draw_texture_part_scale((vita2d_texture *)texture, posX * SCALE,
								   posY * SCALE, startX, startX, sizeX, sizeY,
								   SCALE, SCALE);
}

void HI::mergeTextures(HITexture origin, HITexture destination, short posX,
					   short posY) { // BORKEN
}

void HI::drawRectangle(int posX, int posY, int width, int height,
					   HI::HIColor color) {
...
void HI::endFrame() { vita2d_end_drawing(); }

void HI::swapBuffers() { vita2d_swap_buffers(); }

std::string HI::getDataPath() { return "ux0:/data/RogueVITA/data/"; }

std::string HI::getSavesPath() { return "ux0:/data/RogueVITA/saves/"; }
...
bool HardwareInterface::copyFile(std::string input,
								 std::string output) { // DIRTY BUT NOT BORKEN
...
int HI::getScreenHeight() { return 544 / SCALE; }
int HI::getScreenWidth() { return 960 / SCALE; }

HardwareInterface::HI_PLATFORM HI::getPlatform() { return PLATFORM_PSVITA; }
...
void HI::createThread(void *entrypoint,
	...
	SceUID thread = sceKernelCreateThread(
		"ChunkLoader", (SceKernelThreadEntry) & (entrypoint2.get()), 0x40,
		stack_size, 0, 0, NULL);
...
int HI::getKeysHeld() {
	...
	if (pad.buttons & SCE_CTRL_DOWN)
		keys = (HI_KEYS)((int)keys | HI::HI_KEY_DOWN);
	if (pad.buttons & SCE_CTRL_LEFT)
		keys = (HI_KEYS)((int)keys | HI::HI_KEY_LEFT);
	if (pad.buttons & SCE_CTRL_RIGHT)
		keys = (HI_KEYS)((int)keys | HI::HI_KEY_RIGHT);
	if (pad.buttons & SCE_CTRL_UP)
		keys = (HI_KEYS)((int)keys | HI::HI_KEY_UP);
	if (pad.buttons & SCE_CTRL_START)
		keys = (HI_KEYS)((int)keys | HI::HI_KEY_START);
	if (pad.buttons & SCE_CTRL_CROSS)
		keys = (HI_KEYS)((int)keys | HI::HI_KEY_A);
	if (touch2.reportNum == 1)
		keys = (HI_KEYS)((int)keys | HI::HI_KEY_TOUCH);
	return keys;
}
...
void HardwareInterface::debugPrint(string s, int p) {
	string s2 = s + "\n";
	if (p >= DEBUG_PRIORITY)
		sceClibPrintf(s2.c_str());
}

void HardwareInterface::debugNewLine() { sceClibPrintf("\n"); }

void HI::waitForVBlank() { sceDisplayWaitVblankStart(); }

bool HI::aptMainLoop() { return true; }
#endif